...
SCOPES = ["https://mail.google.com/"]  # IMAP/SMTP/…
...
class GoogleOAuthClient:
    def __init__(self) -> None:
        ...
        self.token_path = os.path.join(TOKEN_DIR, "google_token.json")
    ...
    def login_interactive(self) -> Dict:
        if not self.client_id or not self.client_secret:
            raise RuntimeError("Client ID/secret Google manquants dans config.py")
        ...
        url, state = self.session.create_authorization_url(GOOGLE_AUTH_URL)
    ...
    def get_valid_access_token(self) -> str:
        ...
        if not token:
            raise RuntimeError(
                "Aucun token OAuth2 Google n'est enregistré. Lance d'abord login_interactive()."
            )
